...
def segment_chemical_structures_from_file(
    ...
    """
    Segment all chemical structure depictions found in *file_path*.

    Parameters
    ----------
    file_path : str
        Path to an image or PDF.
    expand : bool, default `True
        Whether to run mask-expansion after Mask R-CNN detection.
    poppler_path : str | None
        Path to Poppler executables on Windows (ignored on Linux).

    Returns
    -------
    list[np.ndarray]
        List of RGBA segments (height × width × 4).
    """

    # 1. Load pages / image ---------------------------------------------------
    if file_path.lower().endswith(".pdf"):
        try:
            pil_pages = convert_from_path(file_path, dpi=300, poppler_path=poppler_path)
        except PDFInfoNotInstalledError:
            fallback = r"C:\Program Files (x86)\poppler-0.68.0\bin"
            pil_pages = convert_from_path(file_path, dpi=300, poppler_path=fallback)
        pages = (np.asarray(p) for p in pil_pages)  # lazy iterator
    else:
        pages = [cv2.imread(file_path)]

    # 2. Process sequentially (no multiprocessing) ---------------------------
    segments: list[np.ndarray] = []
    for page in pages:
        if page is None:  # bad image
            continue
        segments.extend(segment_chemical_structures(page, expand))
